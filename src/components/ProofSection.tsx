    ...
    <section className="mx-auto max-w-6xl px-6 py-24 md:py-32">
      <Reveal>
        ...
        <p className="mt-4 max-w-xl text-text-muted">
          Every screenshot below is from an actual student or Brainbox-run
          store dashboard — unedited, including the slow weeks.
        </p>
      </Reveal>
        ...
        {proofItems.map((item, i) => (
          ...
          >
            <figure className="group relative overflow-hidden border border-border bg-surface">
              ...
              >
                {/* object-contain: proof screenshots are only useful if
                    the numbers are fully legible — never crop real data
                    to force a uniform tile shape. */}
                <Image
                  src={item.src}
                  alt={item.alt}
                  fill
